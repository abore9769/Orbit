...
use orbit_core::zk::experimental::{
...
use orbit_core::zk::stable::{encode_commit_reveal, CommitReveal, COMMIT_REVEAL_TYPE};
use orbit_core::{
...
fn sanctioned_app_module_supports_runtime_registration_flow() {
    let env = Env::default();
    let mut app = orbit_core::app::GameApp::new(&env);
    app.add_systems(orbit_core::app::named_context_system(
        "spawn_player",
        |context| {
    ...
    let query = orbit_core::app::SimpleQueryBuilder::new(&env)
        .with_component(symbol_short!("position"))
        .build();
    assert_eq!(query.execute(app.world(), &env).len(), 1);
}
...
fn sanctioned_app_module_supports_grouped_system_registration() {
    ...
    app.add_systems((
        orbit_core::app::named_system("spawn_player", |world: &mut SimpleWorld, env: &Env| {
            let entity = world.spawn_entity();
            world.set_typed(env, entity, &Position::new(1, 1));
        })
        .in_stage(ScheduleStage::Update)
        .in_set("spawn"),
        orbit_core::app::named_context_system("mark_spawned", |context| {
            let query = orbit_core::app::SimpleQueryBuilder::new(context.env())
                .with_component(symbol_short!("position"))
                .build();
...
fn sanctioned_root_api_supports_archetype_query_builder() {
    let env = Env::default();
    let mut world = orbit_core::ArchetypeWorld::new(&env);
    let entity = world.spawn_entity();
...
fn sanctioned_root_api_exposes_shared_runtime_mut_contract() {
    ...
    let mut archetype = orbit_core::ArchetypeWorld::new(&env);
...
fn sanctioned_root_api_supports_plugin_groups() {
    ...
    impl orbit_core::Plugin for NoopA {
    ...
    impl orbit_core::Plugin for NoopB {
        fn name(&self) -> &'static str {
            "noop_b"
        }
...
fn privacy_namespace_exposes_split_maturity_explicitly() {
    ...
    let encoded = orbit_core::privacy::stable::encode_commit_reveal(&env, &commitment, 55, false);
    assert_eq!(encoded.len(), 41);

    let _exp_fn: fn(
        &Env,
        &orbit_core::privacy::VerificationKey,
        &orbit_core::privacy::Groth16Proof,
        &[orbit_core::privacy::Scalar],
    ) -> Result<bool, orbit_core::privacy::ZKError> =
        orbit_core::privacy::experimental::verify_groth16;
}

#[test]
fn experimental_zk_namespace_exposes_proof_helpers_explicitly() {
    let env = Env::default();
    let g1 = orbit_core::zk::G1Point {
        bytes: BytesN::from_array(&env, &[0u8; 64]),
    };
    let g2 = orbit_core::zk::G2Point {
    ...
    let vk = orbit_core::zk::VerificationKey {
        alpha: g1.clone(),
        beta: g2.clone(),
        gamma: g2.clone(),
        delta: g2,
...
fn accounts_namespace_exposes_curated_beta_entrypoints() {
    ...
    let _verify_fn: fn(
        ...
        &Bytes,
        &BytesN<64>,
    ) -> Result<(), orbit_core::accounts::AccountError> = verify_secp256r1;
}

#[test]
fn auth_namespace_mirrors_beta_accounts_surface() {
    let env = Env::default();
    let _session_builder = orbit_core::auth::SessionBuilder::new(&env)
...
fn ops_namespace_mirrors_standards_surface() {
    let _guard = orbit_core::ops::ExecutionGuard::new(symbol_short!("exec"));
    let _ownable = orbit_core::ops::Ownable::new(symbol_short!("own"));
}
