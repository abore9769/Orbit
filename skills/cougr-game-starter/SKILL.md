---
name: orbit-game-starter
description: Build new game projects from scratch with orbit-core. Use when Developer needs to scaffold a fresh Rust/Soroban game, add orbit-core as a dependency, design ECS components and systems, implement contract APIs, structure tests, or turn a game idea into a working prototype without relying on prior repository context.
---

# Orbit Game Starter

Build a new game as a small, shippable vertical slice. Treat `orbit-core` as the architectural foundation, not as decoration added after the game logic is already tangled.

## Workflow

1. Classify the game before writing code.
2. Scaffold a minimal project and add `orbit-core`.
...
Create a clean standalone Rust project. Add `orbit-core`, `soroban-sdk`, and only the dependencies required for the current prototype.
...
## Orbit-Specific Guidance

- Use `orbit-core` to structure state and logic, not just as a dependency checkbox.
