...
impl StorageWorld {
    ...
    pub fn load_entity(&mut self, env: &Env, entity_id: EntityId) -> Result<(), OrbitError> {
        ...
        let component_types: soroban_sdk::Vec<Symbol> = env
            ...
            .persistent()
            .get(&key)
            .ok_or(OrbitError::EntityNotFound)?;
    ...
    pub fn load_entities(&mut self, env: &Env, entity_ids: &[EntityId]) -> Result<(), OrbitError> {
