...
#[macro_export]
#[doc(hidden)]
macro_rules! __orbit_serialize_field {
...
#[macro_export]
#[doc(hidden)]
macro_rules! __orbit_field_size {
    (i32) => {
        4u32
    };
    (u32) => {
...
#[macro_export]
#[doc(hidden)]
macro_rules! __orbit_deserialize_field {
    ...
    ($struct_name:ident, $symbol:expr, $storage:ident, { $( $field:ident : $ftype:tt ),* $(,)? }) => {
        impl $crate::component::ComponentTrait for $struct_name {
            ...
            fn serialize(&self, env: &soroban_sdk::Env) -> soroban_sdk::Bytes {
                ...
                $(
                    $crate::__orbit_serialize_field!(bytes, env, self.$field, $ftype);
                )*
                bytes
            }

            fn deserialize(_env: &soroban_sdk::Env, data: &soroban_sdk::Bytes) -> Option<Self> {
                let expected_len: u32 = 0 $( + $crate::__orbit_field_size!($ftype) )*;
                if data.len() != expected_len {
                    return None;
                }
                let mut _offset: u32 = 0;
                $(
                    let $field = $crate::__orbit_deserialize_field!(_env, data, _offset, $ftype);
                    _offset += $crate::__orbit_field_size!($ftype);
                )*
                Some(Self { $( $field ),* })
            }
...
/// # Example
/// ```no_run
/// use orbit_core::impl_marker_component;
///
